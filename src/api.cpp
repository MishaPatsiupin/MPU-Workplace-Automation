...
void updateRTCFromNTP() {
    if (WiFi.isConnected()) {
        ...
        if (epochTime > 0) {
            ...
            Serial.println(
                "Ошибка: не удалось получить время через NTP.");
        }
    } else {
        Serial.println(
            "WiFi не подключен. Невозможно обновить время на RTC.");
    }
}
...
void handleGetData() {
    ...
    doc["temperature"] =
        roundToTwoDecimals(now_sensor_data.temperature);
    doc["humidity"] = roundToTwoDecimals(now_sensor_data.humidity);
    doc["pressure"] = roundToTwoDecimals(now_sensor_data.pressure);
    doc["moisture1"] =
        now_sensor_data.moisture1 > 0 ? now_sensor_data.moisture1 : 0;
    doc["moisture2"] =
        now_sensor_data.moisture2 > 0 ? now_sensor_data.moisture2 : 0;
    doc["liquid_sensor_water"] =
        !(now_sensor_data.liquid_sensor_water > 0);
    doc["liquid_sensor_plant"] =
        !(now_sensor_data.liquid_sensor_plant > 0);
...
void handleGetState() {
    ...
    DynamicJsonDocument doc(1024);
    doc["relay_flag"] = relay_settings.relay_flag;
...
void handleSetRelay() {
    Serial.println("Received request: /tepliza/set/relay");
    if (server.hasArg("plain")) {
        DynamicJsonDocument doc(1024);
        deserializeJson(doc, server.arg("plain"));
        if (doc.containsKey("state")) {
            bool relay_flag_local = doc["state"];
            relay_settings.api_relay_set = relay_flag_local;
            relay_control(relay_flag_local);
            relay_settings.relay_flag = relay_flag_local;
            server.send(200, "application/json",
                        "{\"message\":\"Relay upd\",\"new_state\":" +
                            String(relay_settings.relay_flag) + "}");
        } else {
            server.send(400, "application/json",
                        "{\"error\":\"No state provided\"}");
        }
...
void handleGetDataGradka() {
    ...
    doc["pressure"] = roundToTwoDecimals(now_sensor_data.pressure);
    doc["moisture1"] =
        (now_sensor_data.moisture1 + now_sensor_data.moisture2) / 2;
...
void setupServer() {
    if (WiFi.isConnected() && !serverStarted) {
        ...
        server.on("/tepliza/set/relay", HTTP_POST, handleSetRelay);
...
bool send_stop_pomp() {
    String response =
        sendHttpRequest("http://192.168.4.3:80/state/off");
...
bool send_off_relay() {
    String response =
        sendHttpRequest("http://192.168.4.4:80/state/off");
...
unsigned long display_start_time =
    0;  // Время начала отображения информации
void wifi_status() {
    // Проверка состояния кнопки энкодера
    if (show_status) {
        display_start_time =
            millis();  // Сохранение времени начала отображения
        lcd.clear();
        ...
        for (int i = 0; i < stationList.num && i < 3;
             i++) {  // Ограничение на 3 устройства
            wifi_sta_info_t station = stationList.sta[i];
            char macStr[18];
            snprintf(macStr, sizeof(macStr),
                     "%02X:%02X:%02X:%02X:%02X:%02X", station.mac[0],
