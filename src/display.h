...
struct waterind_settings {
    ...
    int max_control_value_auto =
        0;  // Максимальное значение контроля для авто режима
...
extern struct waterind_settings
    waterind_settings;  // Экземпляр структуры для хранения настроек
                        // полива

// Структура настроек освящения
struct relay_settings {
    int type =
        -1;  // Тип управления окном: 0 - отключено, 1 - по времени
    ...
    bool api_relay_set = false;  // Флаг состояния реле для API
};

extern struct relay_settings
    relay_settings;  // Экземпляр структуры для хранения настроек окна
...
extern struct old_data
    old_data;  // Экземпляр структуры для хранения старых данных
...
// Принимает: статус, температура, состояние погоды, влажность 1,
// влажность 2, состояние датчика воды, состояние датчика жидкости в
// растении Возвращает: -
